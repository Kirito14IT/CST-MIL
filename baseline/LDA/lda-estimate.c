...
void run_em(char* start, char* directory, corpus* corpus)
{
    ...
    while (((converged < 0) || (converged > EM_CONVERGED) || (i <= 2)) && (i < EM_MAX_ITER))
    {
        ...
        converged = likelihood_old == 0 ? 1 :
            (likelihood_old - likelihood) / likelihood_old;
        /* Keep the configured per-document iteration budget fixed. */
...
void infer(char* model_root, char* save, corpus* corpus)
{
	...
	for (n = 0; n < doc->length; n++) free(phi[n]);
	free(phi);
...
int main(int argc, char* argv[])
{
    ...
    /* Explicit reproducibility; final optional est argument is the seed. */
    unsigned long seed = argc > 8 ? strtoul(argv[8], NULL, 10) : 42UL;
    seedMT(seed);
