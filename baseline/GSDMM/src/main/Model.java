...
public class Model implements Serializable {
	...
	{ 
		double[] prob = posterior(document);
		for(int k = 1; k < K; k++){
			prob[k] += prob[k - 1];
		}
		...
		return Math.min(kChoosed, K - 1);
	}

	/** Frozen Dirichlet-multinomial posterior; repeated occurrences retain +j. */
	public double[] posterior(Document document) {
		double[] prob = new double[K];
		if (document.wordNum == 0) return prob;
		double max = Double.NEGATIVE_INFINITY;
		for (int k = 0; k < K; k++) {
			double value = Math.log(m_z[k] + alpha);
			int total = 0;
			for (int w = 0; w < document.wordNum; w++) {
				int word = document.wordIdArray[w];
				for (int j = 0; j < document.wordFreArray[w]; j++) {
					value += Math.log(n_zv[k][word] + beta + j)
						- Math.log(n_z[k] + beta0 + total++);
				}
			}
			prob[k] = value;
		...
		double sum = 0;
		for (int k = 0; k < K; k++) sum += (prob[k] = Math.exp(prob[k] - max));
		for (int k = 0; k < K; k++) prob[k] /= sum;
		return prob;
	}
