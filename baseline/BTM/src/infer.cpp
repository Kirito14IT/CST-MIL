...
void Infer::run(string docs_pt, string model_dir, string output_path) {
  load_para(model_dir);
  ...
  string pt = output_path.empty() ? model_dir + "k" + str_util::itos(K) + ".pz_d" : output_path;
...
void Infer::doc_infer_sum_b(const Doc& doc, Pvec<double>& pz_d) {
  ...
  if (doc.size() == 1) {
	if (doc.get_w(0) < 0 || doc.get_w(0) >= pw_z.cols()) return;
	...
	for (int b = 0; b < bs.size(); ++b) {	  
	  ...
	  if (w1 < 0 || w2 < 0 || w1 >= W || w2 >= W) continue;
